...
export default function EntrarForm() {
  ...
  useEffect(() => {
    if (phoneDigits.startsWith('55')) {
      setPhoneDigits((digits) => digits.slice(2));
    }
  }, [phoneDigits]);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setIsLoading(true);
    try {
      const res = await fetch(`/api/users?phone=${encodeURIComponent(phoneDigits)}`);
      if (res.ok) {
        ...
        router.push(
          `/entrar/concluir?callback=${encodeURIComponent(callback)}&phone=${encodeURIComponent(
            phoneDigits,
          )}`,
        );
      } else {
